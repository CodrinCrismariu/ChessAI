...
public:
	...
	std::string getFen(Colour colour);
	int greedyEval(Colour colour);
	ChessBoard();
};

void add(std::string * fen, Piece * k) {
	if(k->colour == Colour::White) {
		char c = k->getSymbol();
		if('a' <= c && c <= 'z')
			c = c + 'A' - 'a';
		fen->push_back(c);
...
int ChessBoard::greedyEval(Colour colour) {
	int a = 0, b = 0;
	for(int i = 0; i < 8; i++)
		for(int j = 0; j < 8; j++) {
			if(squares[i][j] && colour == squares[i][j]->colour)
				a += squares[i][j]->getValue();
		}
	return std::round(double(a * a * a) / (b * b * b));
}

std::string ChessBoard::getFen(Colour colour) {
	std::string fen;
	int pos = 0;
	for(int i = 7, added = 0; i >= 0; i--)
		for(int j = 0; j < 8; j++) {
			if(squares[j][i])
				(added ? fen += added + '0', added = 0 : 0), add(&fen, squares[j][i]);
			else
				added++;
